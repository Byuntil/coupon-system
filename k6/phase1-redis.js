...
const COUPON_CODE = __ENV.COUPON_CODE || 'PHASE1-REDIS-001';
...
// === 선착순 쿠폰 오픈 시나리오 (Phase 1: Redis 전략) ===
// Phase 0 대비 5배 VU → Redis 원자 연산의 처리량·지연 개선 측정
export const options = {
    scenarios: {
        phase1_redis: {
            ...
            stages: [
                ...
                { duration: '30s', target: VUS }, // 경쟁 구간 (Redis 처리량 측정)
    ...
    thresholds: {
        // Redis hot-path이므로 Phase 0보다 훨씬 빠른 응답을 기대
        issue_duration_all:     ['p(99)<5000'],
        issue_duration_success: ['p(95)<100', 'p(99)<300'],
        system_error_rate:      ['rate<0.01'],
    },
};

export function setup() {
    console.log(`=== Phase 1 Redis 부하 테스트 시작 ===`);
    console.log(`Strategy: redis | VUs: ${VUS} | Stock: ${STOCK} | Coupon: ${COUPON_CODE}`);

    const payload = JSON.stringify({
        couponCode: COUPON_CODE,
        couponName: `Phase1 Redis 테스트 쿠폰 (VU=${VUS})`,
        totalStock: STOCK,
    });
...
export default function (data) {
    ...
    if (res.status === 400 || res.status === 409) {
        ...
        sleep(0.05); // 재고 소진 후 tight-loop → 로컬 포트 고갈 방지
        return;
    }
